  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
      ...
      <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
        Nomor yang bisa dihubungi penolong lewat halaman darurat. Maks {MAX_CONTACTS} kontak.
      </p>
      ...
      {atLimit ? (
        <p className="mt-4 rounded-xl bg-slate-50 px-3 py-2.5 text-xs text-slate-500 dark:bg-slate-800/60 dark:text-slate-400">
          Batas maksimal kontak tercapai. Hapus salah satu untuk menambah yang baru.
        </p>
      ) : (
        <form ref={formRef} action={action} className="mt-4 space-y-4 border-t border-slate-100 pt-4 dark:border-slate-800">
          ...
          <label className="flex cursor-pointer items-center gap-2.5">
            ...
            />
            <span className="text-sm text-slate-700 dark:text-slate-300">
              Tampilkan nama di halaman publik
            </span>
          </label>
          ...
          >
            ...
            {pending ? "Menambahkan…" : "Tambah kontak"}
          </button>
        </form>
      )}
    </section>
